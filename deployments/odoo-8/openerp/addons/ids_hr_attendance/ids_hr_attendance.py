...
class hr_attendance(osv.osv):
    ...
    def create(self, cr, uid, vals, context=None):
        """Making single sign/sign out for the day. """
        att_date=date=0
        if 'name' in vals:
            att_date = datetime.strptime(vals['name'],'%Y-%m-%d %H:%M:%S').date()
        d = (datetime.now() + timedelta(hours=5,minutes=30)).date()
        ...
        #if prev_attens:
            #date = datetime.strptime(prev_attens.name,'%Y-%m-%d %H:%M:%S').date()
            #print"date====",date,att_date,vals
        #if prev_attens and prev_attens[0].action == vals['action'] and att_date==date and 'name' in vals:
            #raise osv.except_osv(_('Warning'), _('You are already marked your attendance for the day. Please contact to HR.'))
        #if prev_attens and prev_attens[0].action == vals['action'] and 'name' not in vals:
            #raise osv.except_osv(_('Warning'), _('You are already marked your attendance for the day. Please contact to HR.'))
        ...
        for record in self.pool.get('hr.attendance').search(cr, uid, [('id','=',id)]):
            for data_record in self.pool.get('hr.attendance').browse(cr, uid, record, context=None):
                ...
                department=data_record.employee_id.department_id.id
                cr.execute("update hr_attendance set division_id=%s,department_id=%s where id=%s"%(division,department,id));
        return id
    ...
    def onchange_employee_id(self, cr, uid, ids, division_id,employee_id, context=None):
        ...
        res= {'value': {'division_id': division_id, 
                        ...
                        'location_id': emp_id.office_location.id}}
        return res
...
#   
    def onchange_employee_id(self, cr, uid, ids,employee_id, context=None):
        res= {'value': {'division_id': False,'location_id':False,'department_id':False}}
        """Get Division on change of employee_id. """
        emp_id=self.pool.get('hr.employee').browse(cr, uid, employee_id, context=context)
        res= {'value': {'division_id': emp_id.division.id,'location_id':emp_id.office_location.id,'department_id': emp_id.department_id.id}}
        return res
...
#             if record.name=='General':
    ...
    def create(self, cr, uid, values, context=None):
        ...
        if values['employee_id']:
            emp=self.pool.get('hr.employee').search(cr,uid,[('id','=',values['employee_id'])])
            emp_data=self.pool.get('hr.employee').browse(cr, uid, emp, context=None)
            if emp_data.resource_id.user_id.id==uid:
                raise osv.except_osv(_('Warning'), _('You can not mark your own attendance. Please contact to HR Manager.'))
    ...
    def write(self, cr, uid,ids,vals, context=None):
        ...
        emp=self.search(cr,uid,[('id','=',ids)])
        emp_data=self.browse(cr, uid, emp, context=None)
        if emp_data.employee_id.user_id.id==uid:
            raise osv.except_osv(_('Warning'), _('You can not modify your own attendance. Please contact to HR Manager.'))
    ...
    def markmiss_attendance(self,cr,uid,ids=False,context=None):
        ...
        for record in self.pool.get('hr.attendance').search(cr, uid, [('flag','=',False)], order='employee_id,name ASC'):
            for data_record in self.pool.get('hr.attendance').browse(cr, uid, record, context=None):
                ...
                date_ist = date_time + timedelta(hours=5,minutes=30)
                atten_date = datetime.strftime(date_ist,'%Y-%m-%d %H:%M:%S')
                punch=datetime.strptime(atten_date,'%Y-%m-%d %H:%M:%S').date()
                employee=data_record.employee_id.id
                ...
                if data_record.action=="sign_in":
                    ...
                    if data_in:
                        ...
                        if data1_in.worked_hours == 0.0:
                            ...
                            res = self.pool.get('final.punch').write(cr, uid, [punch_id1], vals, context=context)
                            cr.execute("update final_punch set state='',flag=False WHERE id=%s"%punch_id1);
                            cr.execute("update hr_attendance set flag=True where id=%s"%data_id);
                        else:
                            cr.execute("Insert into final_punch (employee_id,date,shift,location_id,division_id,department_id,out_time)Values (%s,%s,1,%s,%s,%s,%s)",(employee,punch,location,data_record.division_id.id,data_record.department_id.id,date2));
                            cr.execute("update hr_attendance set flag=True where id=%s"%data_id);
                    else:
                        print"else==========="
                        cr.execute("Insert into final_punch (employee_id,date,shift,location_id,division_id,department_id,out_time)Values (%s,%s,1,%s,%s,%s,%s)",(employee,punch,location,data_record.division_id.id,data_record.department_id.id,date2));
    ...
    def mark_attendance(self,cr,uid,ids=False,context=None):
        ...
        for data_ids in biometric_data.search(cr, uid, [('flag','=',False)]):
            for data_pool in biometric_data.browse(cr, uid, data_ids, context=context):
                ...
                t1=str(time1)
                a=cr.execute("SELECT id,division,department_id from hr_employee WHERE biometric_code=%s"%e_code);
                c = cr.fetchall()
                list = [int(i[0]) for i in c]
                list1 = [int(i[1]) for i in c]
                list2 = [int(i[2]) for i in c]
                if len(list) <> 0:
                    data = list[0]
                    data1 = list1[0]
                    data2 = list2[0]
                    ...
                    cr.execute("update biometric_data set flag=True where id=%s"%data_pool.id);
                else:
                    pass
        return True
    
    def mark_attendance_mohali(self,cr,uid,ids=False,context=None):
        """Gets data from biometric.data.mohali and updates into hr.attendance. """
        biometric_data_mohali = self.pool.get('biometric.data.mohali')
        atten_pool = self.pool.get('hr.attendance')
        res={}
        data=data1=0
        action=''
        for data_ids in biometric_data_mohali.search(cr, uid, [('flag','=',False)]):
            for data_pool in biometric_data_mohali.browse(cr, uid, data_ids, context=context):
                emp_code=data_pool.emp_code
                e_code="'" +str(emp_code)+ "'"
                date = data_pool.name
                print"datemohali=================",date
                if data_pool.status=='IN':
                    action='sign_in'
                else:
                    action='sign_out'
                a=cr.execute("SELECT id,division,department_id from hr_employee WHERE biometric_code=%s"%e_code);
                ...
                if len(list) <> 0:
                    ...
                    att_id=atten_pool.create(cr,uid,res,context=context)
                    cr.execute("update biometric_data_mohali set flag=True where id=%s"%data_pool.id);
                else:
                    pass
        
        return True
    
    def mark_attendance_noida(self,cr,uid,ids=False,context=None):
        """Gets data from biometric.data.noida and updates into hr.attendance. """
        biometric_data_noida = self.pool.get('biometric.data.noida')
        atten_pool = self.pool.get('hr.attendance')
        res={}
        data=data1=0
        action=''
        for data_ids in biometric_data_noida.search(cr, uid, [('flag','=',False)]):
            for data_pool in biometric_data_noida.browse(cr, uid, data_ids, context=context):
                emp_code=data_pool.emp_code
                e_code="'" +str(emp_code)+ "'"
                date = data_pool.name
                print"datenoida=================",date
                if data_pool.status=='IN':
                    action='sign_in'
                else:
                    action='sign_out'
                ...
                if len(list) <> 0:
                    ...
                    att_id=atten_pool.create(cr,uid,res,context=context)
                    cr.execute("update biometric_data_noida set flag=True where id=%s"%data_pool.id);
                else:
                    pass
